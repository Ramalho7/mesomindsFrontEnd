...
function RouteComponent() {
  const { simulado } = Route.useParams()
  const router = useRouter()
  const { data: simuladoData, isLoading, isError } = useGetQuestionCollectionById(Number(simulado))
  ...
  if (isError || !simuladoData) {
    return <div className="text-red-500">Erro ao carregar o simulado.</div>
  }

  const questions = simuladoData.questions || []
  const currentQuestion = questions[currentQuestionIndex]

  console.log("Simulado Data:", simuladoData)
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-4">
        <div className="max-w-2xl w-full bg-white border-2 border-secondary rounded-lg p-8 text-center">
          <h1 className="text-3xl font-bold text-secondary mb-6">Simulado Finalizado!</h1>
          <div className="mb-8">
  ...
  if (!currentQuestion) {
    return (
      <div className="flex justify-center items-center h-screen">
        <p className="text-lg text-red-500">Nenhuma questão disponível neste simulado.</p>
      </div>
    )
  }
    ...
    <div className="flex flex-col min-h-screen px-4 py-10">
      <div className="max-w-4xl w-full mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-bold text-secondary">{simuladoData.title}</h1>
